...
def run_test_generation(source_code_loc: str, class_name: str, class_description: str, max_iterations: int,
    ...
    """
    logger.info("=== 启动多模型协作 Java 单元测试生成 ===\n")
    ...
    logger.info(f"初始目标覆盖率: {current_feedback['iterationAdvice']['targetNextCoverage']}\n")
    ...
    while iteration < max_iterations:
        ...
        logger.info(f"  质量评分: {quality_score}, 覆盖充分性: {coverage_adequacy}")
        logger.info(f"  是否继续迭代: {should_continue}")
        ...
        logger.info(f"  下一轮目标覆盖率: {current_feedback['iterationAdvice'].get('targetNextCoverage', '未指定')}\n")

    if iteration >= max_iterations:
        logger.info(f"\n达到最大迭代次数 ({max_iterations})，停止生成。")
    logger.info("=== 测试生成流程结束 ===")
